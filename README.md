...
- 정답 한/영 변환
    - 표시되는 정답을 영어 또는 한글 선택
    - Dictionary 구조를 이용하여 한/영 동음이의어 모두 정답으로 처리
...
- 추후에 온라인 멀티플레이로 발전시킬 의향이 있습니다.
- 사운드 요소가 다양하지 않아서 추가 sfx를 넣을 예정입니다.
- 게임 오버 화면에서 유저가 그린 그림들과 AI로 생성된 그림들을 나란히 대조하여 전시하는 요소가 들어갔으면 더 재미를 느낄 수 있을 것 같아서 이것 또한 추후 구현 예정입니다.

### Model
- sketch clssification 모델의 성능이 매우 좋지는 않아, 게임 내 category 개수를 50개로 제한하였습니다. (test_acc: 0.7715)
- 그럼에도 높은 오분류율로 인해 사용자의 스케치를 오분류하여 완전히 다른 그림을 생성하는 경우도 존재합니다(하지만 사용자의 그림이 오분류 때문인지 그냥 못 그려서 그런건지 알 수 없기 때문에 재미 요소가 됩니다).
